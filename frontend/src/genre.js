...
class Genre {
  ...
  render() {
    return `
        ...
        <button data-id="${this.id}" class="editGenre" onclick="Genre.editGenre(${this.id})"; return false;>Edit Genre Name</button>
        <button data-id="${this.id}" class="deleteGenre" onclick="Genre.deleteGenre(${this.id})"; return false;>Delete Genre</button>
        </div>
        `;
  }
      ...
      .then(genre => {
        ...
        editGenre.addEventListener("click", (e) => {
          ...
          if (name == "" || name == null) {
            ...
            Genre.updateGenre(e.currentTarget.dataset.id);
            e.preventDefault();
            formArea.style.display = "none";
          }
        });
  ...
  static updateGenre(id) {
    ...
    fetch(BASE_URL + `/genres/${id}`, {
      ...
      .then(resp => resp.json())
      .then(genre => {
        let content = document.querySelectorAll(`div a[data-id="${genre.id}"]`)[0]
          .parentElement;
        content.innerHTML = `
          ...
          <button data-id="${genre.id}" class="editGenre" onclick="Genre.editGenre(${genre.id})"; return false;>Edit Genre Name</button>
          <button data-id="${genre.id}" class="deleteGenre" onclick="Genre.deleteGenre(${genre.id})"; return false;>Delete Genre</button>
          `;
          newBookBtn()
          genreBooksLink()
      });
  ...
  static deleteGenre(id) {
    const confirmation = confirm(
      "Are you sure you want to delete this genre? All book log entries in this genre will be deleted as well."
    );
    if (confirmation) {
      fetch(BASE_URL + `/genres/${id}`, {
        method: "DELETE",
        ...
        document
          .querySelectorAll(`div a[data-id="${id}"]`)[0]
